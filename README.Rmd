...
analytical methods. The typical sequence is as follows: first, apply
propensity score (PS) methodology to pre-select a subset of real-world data,
including patients similar to those in the current clinical study with
respect to covariates. Then, stratify both the selected patients and those
in the current study into more homogeneous strata. Next, apply methods such
as the power prior approach or composite likelihood approach within each
stratum to draw inference for the parameters of interest. This package
provides functions implementing the PS-integrated RWE analysis methods
proposed in
