...
#'   \code{cur_grp_level = 1}.
#' @param v_covs Vector of column names corresponding to covariates.
#' @param v_arm Column name corresponding treatment vs. control in two-arm
...
#'
#' @return A list of class \code{RWE_DWITHPS} with items:
#'
#' \itemize{
...
#' @title Summarize PS scores
#'
#' @description Get number of subjects and the distances of PS distributions for
#'   each PS strata.
#'
#' @inheritParams rwe_ps
#' @param data_withps A list of class \code{RWE_DWITHPS} that is generated using
...
#' @param ... Additional parameters for \code{\link{get_distance}}. For example,
#'   \code{metric} with options such as overlapping area (\code{ovl}).
#'
#' @return A data frame of class \code{RWE_PSDIST} with columns:
#'   \itemize{
#'     \item{Strata}{Index of stratum. 0 represents the overall information.}
#'     \item{N0, N1}{Number of subjects in group 0 and 1.}
#'     \item{N00, N10}{Number of arm 0 subjects in group 0 and 1, when arm
#'                     exists.}
#'     \item{N01, N11}{Number of arm 1 subjects in group 0 and 1, when arm 
#'                     exists.}
#'     \item{Dist}{Distance measure.}
#' }
#'
#' @examples
...
#'
#' @export
rwe_ps_dist <- function(data_withps,
                        min_n0 = 10,
                        trt_arm_level = NULL,
                        ...) {
...
#' @title PS matching
#'
#' @description Match patients in external data source with patients in current study based
#' on PS using nearest neighbor method.
#'
#' @inheritParams rwe_ps
#' @param dta_cur Current study data frame.
#' @param dta_ext External data source data frame.
#' @param ratio Matching ratio with default value 3 meaning 3:1 matching.
#'
#' @return A matrix with two columns:
#' \itemize{
#'   \item{code{pid}}: ID in \code{dta_cur}.
#'   \item{\code{match_id}}: matched ID from \code{dta_ext}.
#' }
#'
#' @examples
...
rwe_ps_match <- function(dta_cur, 
    ...
    # Output
    out <- cbind(pid      = rep(1:n_cur, each = ratio),
                 match_id = rst)
    return(out)
}
