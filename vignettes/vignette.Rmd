...
**ps_rwe_powerp** allows one to conduct the analysis proposed in Wang et. al.
...
```{r, eval=T, echo=TRUE}
ps_bor <- ps_rwe_borrow(dta_ps, total_borrow = 40,
                        method = "distance")
rst_pp <- ps_rwe_powerp(ps_bor, v_outcome = "Y_Bin",
                        outcome_type = "binary")
```
...
For single arm studies when there is one external data source, the function
**ps_rwe_cl** allows one to conduct the analysis proposed in Wang et. al.
...
```{r, eval=T, echo=TRUE}
rst_cl <- ps_rwe_compl(ps_bor, v_outcome = "Y_Bin",
                       outcome_type = "binary")
summary(rst_cl)
```
...
*control* subjects, the function **ps_rwe_cl2arm** allows one to conduct the
...
```{r, eval=T, echo=TRUE}
data(ex_dta_rct)
dta_ps_rct <- ps_rwe_est(ex_dta_rct, v_covs = paste("V", 1:7, sep = ""),
...
rst_cl_rct <- ps_rwe_compl(ps_bor_rct, v_outcome = "Y_Con",
                           outcome_type = "binary")
