...
```{r, eval=T, echo=TRUE}
ps_bor <- rwe_ps_borrow(dta_ps, total_borrow = 40,
                        method = "distance")
rst_pp <- rwe_ps_powerp(ps_bor, v_outcome = "Y_Bin",
                        outcome_type = "binary")
```
...
```{r, eval=T, echo=TRUE}
rst_cl <- rwe_ps_compl(ps_bor, v_outcome = "Y_Bin",
                       outcome_type = "binary")
summary(rst_cl)
```
...
```{r, eval=T, echo=TRUE}
data(ex_dta_rct)
dta_ps_rct <- rwe_ps_est(ex_dta_rct, v_covs = paste("V", 1:7, sep = ""),
...
rst_cl_rct <- rwe_ps_compl(ps_bor_rct, v_outcome = "Y_Con",
                           outcome_type = "binary")
