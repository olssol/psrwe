...
### PS matching.
dta_ps_match <- ps_rwe_match(dta_ps_single, ratio = 2, strata_covs = "V1")
dta_ps_match
...
ps_bor_match <- ps_rwe_borrow(dta_ps_match, total_borrow = 30)
ps_bor_match

### PSCL, single arm study, binary outcome.
rst_cl <- ps_rwe_compl(ps_bor_match,
                       outcome_type = "binary",
                       v_outcome    = "Y_Bin")
rst_cl
...
ps_bor_match_opt <- ps_rwe_borrow(dta_ps_match_opt, total_borrow = 30)
rst_cl_opt <- ps_rwe_compl(ps_bor_match_opt,
                           outcome_type = "binary",
                           v_outcome    = "Y_Bin")
rst_cl_opt
